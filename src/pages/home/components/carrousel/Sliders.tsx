...
export default function Sliders({ index }: SlidersProps) {
    ...
    const { setSliderIndex, isMobile } = useAppStore();
    ...
    const sliderPosition = () => {
        ...
        if (isMobile) {
            positionsByIndex = [
                "-translate-x-[30%] z-10 skew-y-3 scale-75", // Index 1
                "-translate-x-[15%] z-30 skew-y-3 scale-90", // Index 2
                "translate-x-0 z-50 scale-100", // Index 3
                "translate-x-[15%] z-40 -skew-y-3 scale-90", // Index 4
                "translate-x-[30%] z-20 -skew-y-3 scale-75", // Index 5
            ];
        } else {
            positionsByIndex = [
                "-translate-x-[30%] z-10 skew-y-3 scale-75", // Index 1
    ...
    return (
        <div
            ...
            className={`w-3/6 md:h-[250px] h-[150px] rounded-2xl bg-cover bg-center p-2
        ...
        >
            {sliderIndex === index && (
                <>
                    ...
                    <p className="text-xs col-start-1 w-2/3 z-[70] md:block hidden">
                        {game.description}
                    </p>
                </>
            )}
